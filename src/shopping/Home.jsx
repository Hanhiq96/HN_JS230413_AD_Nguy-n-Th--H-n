...
import HomeComp from "./HomeComp.css";
import Cart from "./Cart";
function Home() {
  const [datas, setDatats] = useState([
    {
  ...
  const handleIncrement = (index) => {
    const updatadDatas = [...datas];
    updatadDatas[index].quantity += 1;
    setDatats(updatadDatas);
  };

  const handleDecrement = (index) => {
    const updatadDatas = [...datas];
    if (updatadDatas[index].quantity > 1) {
      updatadDatas[index].quantity -= 1;
      setDatats(updatadDatas);
    }
  };
  ...
  return (
    <div>
         {isCartVisible && (
        <div>
          <Cart products={cartItems} deleteProduct={handleDeleteProduct} />
        </div>
      )}
      <div className="header">
        <div className="title">
          <span style={{ marginRight: "20px" }}>Home</span>
          <span onClick={handleCartToggle}>Cart</span>
          <h1>List Products</h1>
        </div>
      </div>
